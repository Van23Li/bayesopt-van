...
fontTimes = false; % true:使用Times; false:不使用Times
...
if fontTimes
    ...
    set(legend1, 'FontWeight','normal'); % 设置legend字体
end
...
if fontTimes
    ...
    title(titleName(subst:end),'fontsize',fontSize,'fontweight','bold'); % 图片标题
    xlabel(xLabelName,'fontsize',fontSize,'fontweight','bold'); % 横轴名称
    ylabel(yLabelName,'fontsize',fontSize,'fontweight','bold'); % 纵轴名称
