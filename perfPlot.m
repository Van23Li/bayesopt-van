...
% 可选项
paperTitle = 'NCAA_attribute'; % 针对的会议或期刊名称和作者 NCAA_attribute NCAA_deep NCAA_HC
...
figWidth = 750; % 图片宽度 2200
figHeight = 400; % 图片高度 560
...
for i=1:length(metricTypeSet)
    ...
    for j=1:length(evalTypeSet)
        ...
        figure;
%         ICRA_plotDrawSave(numTrk,plotDrawStyle,aveSuccessRatePlot,idxSeqSet,rankNum,rankingType,rankIdx,nameTrkAll,thresholdSet,titleName, xLabelName,yLabelName,paperTitle,evalType);
...
%             saveas(gcf, [figsNameTosave, '.png']); % 保存成 png，或 tiff
        ...
        if AttPerfPlot == false % false true 才会画各属性图
            continue; 
        end
